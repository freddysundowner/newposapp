...
class Purchases {
  createPurchase({required Map<String, dynamic> body, required shopId}) async {
    var response = await DbBase()
        .databaseRequest(product+"updatepurchases/${shopId}", DbBase().patchRequestType, body: body);
    return jsonDecode(response);
  }

  getPurchase({ required shopId}) async {
    var response = await DbBase().databaseRequest(product+"purchase/${shopId}", DbBase().getRequestType);
    return jsonDecode(response);
  }
}
